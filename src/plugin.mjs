 ...
 * @param {boolean} [vFlip]
 */
function iconTransform(body, left, top, width, height, rotate, hFlip, vFlip) {
  const transform = []
 ...
 * @prop {string} [mask] - Mask icon utility prefix
 * @prop {string} [background] - Background icon utility prefix
 */

/**
 * @typedef {object} Options
 * @prop {string} [config=icons.json] - The icons config name resolved from the project root.
 */
 ...
 * @param {string} root
 */
async function transformConfig(config, root) {
  try {
    await stream.pipeline(
      loadIconData(config, root),
      transform,
 ...
 * @param {string} root
 */
async function* loadIconData(config, root) {
  for (const description of config.iconSets) {
 ...
 */
async function _loadIconData(description, root) {
...
/**
 * @param {AsyncGenerator<IconData>} source
 * @returns {AsyncGenerator<TransformIconData>}
 */
 ...
 */
function writeToFile(prefix) {
  const { mask = 'mi', background = 'bi' } = prefix

  /**
   * @param {AsyncGenerator<TransformIconData>} source
   */
  return async source => {
    ...
    try {
      ...
      for await (const { iconSetName, icons } of source) {
        let resolveWriteTheme

        const writeThemePromise = new Promise(resolve => {
          resolveWriteTheme = resolve
        })
        ...
        let resolveWriteInlineTheme

        const writeInlineThemePromise = new Promise(resolve => {
          resolveWriteInlineTheme = resolve
        })
